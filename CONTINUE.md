...
- 正常启动端口为 `8000`；今日为真实 API 和 Playwright 验证临时启动过服务，验证后已关闭，
  当前未保留本轮测试服务。
...
- `ruff check .`：通过。
- `pytest -q`：`101 passed`，仅有一条第三方 `StarletteDeprecationWarning`。
...
- 量化 UI 离线 Playwright：1440px 与 390px 均无横向溢出或控制台错误；已覆盖
  Factor 默认折叠、波浪三情景文案、审计区展开和自动刷新缩放恢复。
...
3. FastAPI TestClient 产生第三方 Starlette/httpx 弃用警告，不影响当前 101 项测试结果；升级相关依赖前需确认兼容性。
...
3. 由用户重启平台后在常用浏览器中验收黄色 Trigger、浪形 Top-1 连线、三情景折线、
