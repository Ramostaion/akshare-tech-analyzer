...
- `gann/` V2.1 同时维护上、下行右侧确认 Pivot 候选；同方向的新 Pivot 达到 1 ATR 摆幅并完成右侧三根确认后晋升为当前主锚，旧锚只保留为不参与当前扇线绘制的长期参考，避免锚点在单次波动中漂移或在长期趋势中僵化。`2×1`、`1×1`、`1×2` 从当前主锚按固定单位连续延伸，不从最新收盘价重新起线。尺度只在锚点 `ATR14/8`、长期 ATR 中位数和前一摆幅速度三个预设候选中按已发生数据选择，并公开贴合结果。模型识别加速、保持、降速、连续失守和收复事件，仅在角线、时间窗与价格分割三项重叠时显示时价共振区；屏幕角度、周期窗口和共振区均不是精确价格或日期预测。历史验证在新主锚确认时结束旧生命周期，分开统计方向确认、`1×1` 触及/守住、角线失守、目标结果及 MFE/MAE；江恩只作为当前 Trigger 的速度支持或冲突证据，不独立发单。
