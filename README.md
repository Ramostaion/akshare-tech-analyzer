...
- `wyckoff/` 第一版使用前置 60 根 K 线的因果分位区间、ATR、价差和相对成交量识别 Spring、UTAD、SOS、SOW、LPS、LPSY，给出吸筹/派发候选、Phase B～E、收盘确认位、目标观察区和结构失效位。历史统计采用逐根前缀回放且同根目标/失效按失效优先；结构匹配度不是概率，已决样本少于 30 次时不展示命中率。成交量缺失或不可靠时明确停用该模型，不以纯价格走势伪造威科夫量价结构。
- K 线图上方的“波浪理论”“江恩理论”和“威科夫理论”按钮独立控制整套算法 trace、shape 与 annotation；切换不会重新请求行情或重置 Plotly 缩放，自动刷新会保留图层开关状态。默认开启波浪图层、关闭江恩和威科夫图层，以控制信息密度。图表首次加载默认聚焦近期 K 线（日线 220 根、周线 156 根、月线 120 根、分钟线 300 根），价格轴覆盖该窗口高低点及当前波浪/江恩/威科夫相关价位，完整历史仍可平移查看；右侧为条件路径预留至少 15% 的示意空间，该比例只影响显示，不改变目标价格、失效位或历史回放周期。主 K 线区提供类似 TradingView 的十字光标，底部显示对应日期/时间，右侧显示对应价格；拖动或离开图表时自动隐藏。
