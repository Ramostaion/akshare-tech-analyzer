...
- 新增时间序列研究工具、Elliott Wave Top-N 候选、ATR 归一化江恩条件路径和威科夫量价结构候选；所有预测图层均带确认与失效条件。
...
- `wave/` 使用短/标准/宽三种尺度的右侧确认 Swing、ATR ZigZag 和硬规则输出 Top-N 候选，覆盖上涨与下跌推动浪、进行中/已完成推动浪及 ABC。Fib 只参与结构匹配度评分，该分数不是方向概率。K 线图仅连接匹配度最高候选的已确认 Pivot；进行中结构展示条件目标区与收盘失效位，已完成结构展示下一阶段反向观察区。图中用“确认后延续”“尝试失败后失效”和“确认前震荡等待”表达三类条件路径，同时保留水平目标带、确认位、失效位和随路径扩张的 ATR 不确定性走廊；折线节点及横向距离仅为结构示意，不预测具体价格或到达时间。没有合法候选时，图内会明确提示原因。历史同类情景采用逐根无未来回放，收盘确认当根不能倒推使用盘中目标触及；已决样本少于 30 次时不展示概率。
- `gann/` V3.0 是从零重建的 Gann Price-Time 引擎。锚点来自 ATR 与百分比混合阈值的右侧确认 Pivot，并按强度、摆幅、ATR 显著性、水平位、成交量、动量反转和持续时间形成 100 分 Anchor Score；上、下行候选独立维护。默认 `1×1 = 0.25 × ATR(14) / bar`，同时支持百分比尺度和对数价格尺度，角线位置与屏幕尺寸无关。价格模块组合八分位、Fibonacci、已确认水平位、均线和 BOLL；时间模块从近期真实摆幅时长推导 `0.5T / 1T / 1.5T / 2T / 3T` 透明观察窗。只有角线、价格因素与时间窗共同接近时才输出 Price-Time Confluence。每个条件情景均提供 Trigger、连续收盘确认、目标区、时间窗、失效规则和期限衰减后的未校准相对置信度。日线最多绘制 15 根主投影，周线最多 6 根；更远范围不画精确路径。历史验证按确认时间和锚点生命周期回放角线事件，报告 1/3/5/10/20 根收益、MFE/MAE、时间窗反转率及等样本随机基线，并提供严格时间顺序的样本外拆分和独立参数校准入口。日线与周线分别识别 Pivot；预测快照以只增不改方式保存到 SQLite，过去快照不会被未来重算覆盖。江恩仍只作为当前 Trigger 的支持或冲突证据，不独立发单。
- `wyckoff/` V2 使用逐根因果状态机维护冻结交易区间；只有连续三根有效离开才重建区间，单根 Spring/UTAD 式越界不会移动历史边界。模型同时保留吸筹与派发候选，按 SC/BC、AR、ST、Spring/UTAD、Test、SOS/SOW、LPS/LPSY 的顺序约束推断 Phase A～E，并区分收盘确认与后续行为确认。结构匹配度拆分为区间稳定度、事件顺序、量价质量、后续确认、背景加分和冲突扣分，仍明确不是概率。Phase C 使用 Spring/UTAD 极值作为失效基准，Phase D/E 优先使用 LPS/LPSY 或突破带；历史统计按冻结区间生命周期去重，分开报告结构确认率与确认后目标先达率，同根目标/失效仍按失效优先。成交量缺失或不可靠时明确停用该模型，不以纯价格走势伪造威科夫量价结构。
- K 线图上方的“波浪理论”“江恩理论”和“威科夫理论”按钮独立控制整套算法 trace、shape 与 annotation；切换不会重新请求行情或重置 Plotly 缩放，自动刷新会保留图层开关状态。默认开启波浪图层、关闭江恩和威科夫图层，以控制信息密度。图表首次加载默认聚焦近期 K 线（日线 220 根、周线 156 根、月线 120 根、分钟线 300 根），价格轴覆盖该窗口高低点及默认可见的波浪价位；默认隐藏的江恩与威科夫远端投影不参与初始取景，避免压扁 K 线，完整历史和超出视窗的投影仍可手动平移缩放查看。右侧为条件路径预留至少 15% 的示意空间，该比例只影响显示，不改变目标价格、失效位或历史回放周期。主 K 线区提供类似 TradingView 的十字光标，底部显示对应日期/时间，右侧显示对应价格；拖动或离开图表时自动隐藏。
- 波浪情景路径先用灰色表示尚未分叉的共同观察段，到确认位或调整浪目标区入口后，才分别绘制绿色延续与红色失效路径。绿色 ATR 不确定性走廊使用逐段四边形填充，不使用可能自交成“8 字形”的整体折线填充。
